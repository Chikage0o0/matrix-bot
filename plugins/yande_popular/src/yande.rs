...
pub async fn get_image_list(url: &str) -> Result<Vec<i64>> {
...
pub async fn get_download_list(image_list: &[i64], db: &DB) -> Result<ImgInfo> {
    let mut download_list = HashMap::new();

    for img_id in image_list {
        if db.contains(&img_id.to_string())? {
        ...
        if img_data.score < 50 || db.contains(&id.to_string())? {
...
pub async fn download_img(id: i64, url: &str, path: &Path) -> Result<PathBuf> {
    let client = CLIENT.get_or_init(client_builder);
    ...
    let path = path.join(format!("{}.{}", id, ext));
...
mod tests {
    ...
    async fn test_get_image_list() {
        let resp = get("https://yande.re/post/popular_recent").await.unwrap();
        let image_list = get_image_list(&resp).await.unwrap();
        println!("{:#?}", image_list);
